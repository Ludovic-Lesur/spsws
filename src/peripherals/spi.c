...
void SPI2_PowerOn(void) {
	...
	GPIO_Configure(GPIO_SPI2_MOSI, AlternateFunction, PushPull, HighSpeed, NoPullUpNoPullDown);
	GPIO_Configure(GPIO_SPI2_MISO, AlternateFunction, PushPull, HighSpeed, NoPullUpNoPullDown);
}
...
void SPI2_WriteShort(unsigned short tx_data) {
	...
	*((volatile unsigned short*) &(SPI2 -> DR)) = tx_data;
	// Wait for transmission to complete.
	while ((((SPI2 -> SR) & (0b1 << 1)) == 0) || (((SPI2 -> SR) & (0b1 << 7)) != 0)); // Wait for TXE='1' and BSY='0'.
}
...
void SPI2_ReadShort(unsigned short tx_data, unsigned short* rx_data) {
	...
	*((volatile unsigned short*) &(SPI2 -> DR)) = tx_data;
	// Wait for incoming data.
	while (((SPI2 -> SR) & (0b1 << 0)) == 0); // Wait for RXNE='1'.
	(*rx_data) = *((volatile unsigned short*) &(SPI2 -> DR));
	// Wait for reception to complete.
	while ((((SPI2 -> SR) & (0b1 << 0)) != 0) || (((SPI2 -> SR) & (0b1 << 7)) != 0)); // Wait for RXNE='0' and BSY='0'.
}
#endif
