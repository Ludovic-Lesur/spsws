...
void GPIO_SetMode(GPIO gpio, GPIO_Mode mode) {
	...
	if ((gpio.num >= 0) && (gpio.num < GPIO_PER_PORT)) {
		...
		case Input:
			// MODERy = '00'.
			gpio.port -> MODER &= ~(0b1 << (2*gpio.num));
			gpio.port -> MODER &= ~(0b1 << (2*gpio.num+1));
			break;
		case Output:
			// MODERy = '01'.
			gpio.port -> MODER |= (0b1 << (2*gpio.num));
...
GPIO_Mode GPIO_GetMode(GPIO gpio) {
	unsigned char bit0 = ((gpio.port -> MODER) & (0b1 << (2*gpio.num))) >> (2*gpio.num);
	unsigned char bit1 = ((gpio.port -> MODER) & (0b1 << (2*gpio.num+1))) >> (2*gpio.num+1);
...
void GPIO_SetOutputSpeed(GPIO gpio, GPIO_OutputSpeed output_speed) {
	...
	if ((gpio.num >= 0) && (gpio.num < GPIO_PER_PORT)) {
		...
		case LowSpeed:
			// OSPEEDRy = '00'.
			gpio.port -> OSPEEDR &= ~(0b1 << (2*gpio.num));
			gpio.port -> OSPEEDR &= ~(0b1 << (2*gpio.num+1));
			break;
		case MediumSpeed:
			// OSPEEDRy = '01'.
			gpio.port -> OSPEEDR |= (0b1 << (2*gpio.num));
...
void GPIO_SetPullUpPullDown(GPIO gpio, GPIO_PullResistor pull_resistor) {
	...
	if ((gpio.num >= 0) && (gpio.num < GPIO_PER_PORT)) {
		...
		case NoPullUpNoPullDown:
			// PUPDRy = '00'.
			gpio.port -> PUPDR &= ~(0b1 << (2*gpio.num));
			gpio.port -> PUPDR &= ~(0b1 << (2*gpio.num+1));
			break;
		case PullUp:
			// PUPDRy = '01'.
			gpio.port -> PUPDR |= (0b1 << (2*gpio.num));
	...
	if ((af_num >= 0) && (af_num < AF_PER_GPIO)) {
		...
		if ((gpio.num >= 0) && (gpio.num < AFRH_OFFSET)) {
			...
			for (i=0 ; i<4 ; i++) {
				if (af_num & (0b1 << i)) {
					// Bit = '1'.
					gpio.port -> AFRL |= (0b1 << (4*gpio.num+i));
				}
				else {
					// Bit = '0'.
					gpio.port -> AFRL &= ~(0b1 << (4*gpio.num+i));
			...
			if ((gpio.num >= AFRH_OFFSET) && (gpio.num < GPIO_PER_PORT)) {
				...
				for (i=0 ; i<4 ; i++) {
					if (af_num & (0b1 << i)) {
						// Bit = '1'.
						gpio.port -> AFRH |= (0b1 << (4*(gpio.num-AFRH_OFFSET)+i));
					}
					else {
						// Bit = '0'.
						gpio.port -> AFRH &= ~(0b1 << (4*(gpio.num-AFRH_OFFSET)+i));
...
unsigned char GPIO_Read(GPIO gpio) {
	...
	if ((gpio.num >= 0) && (gpio.num < GPIO_PER_PORT)) {
		...
		case Input:
			...
			if (((gpio.port -> IDR) & (0b1 << gpio.num)) != 0) {
				state = 1;
			}
			break;
		case Output:
			// GPIO configured as output -> read ODR register.
			if (((gpio.port -> ODR) & (0b1 << gpio.num)) != 0) {
				state = 1;
			}
			break;
		default:
