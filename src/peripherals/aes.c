/*
 * aes.c
 *
 *  Created on: 19 juin 2018
 *      Author: Ludovic
 */

#include "aes.h"

/*** AES local macros ***/

#define xtime(a) 		(((a) < 0x80) ? ((a) << 1) : (((a) << 1) ^ 0x1b))

/*** AES global variables ***/

const unsigned char STable[256] = {
	
};

/*** AES functions ***/

void AES_Encode(unsigned char* block, unsigned char* key) {
	unsigned char _roundCounter;
	unsigned char _rcon;
	unsigned char i_aes;
	unsigned char temp_aes;

	/* Initiate round counter = 10 */
	_roundCounter = 10;
	_rcon = 0x01;

	/* Key addition */
	for (i_aes=0 ; i_aes<AES_BLOCK_SIZE ; i_aes++) {
		block[i_aes] ^= key[i_aes];
	}

	do {
		/* STable substitution */
		for (i_aes=0 ; i_aes<AES_BLOCK_SIZE ; i_aes++) {
			block[i_aes] = STable[block[i_aes]];
		}

        /* First row (row 0) unchanged */

        /* Second row (row 1) shifted left by one */
        temp_aes = block[1];
        block[1] = block[5];
        block[5] = block[9];
        block[9] = block[13];
        block[13] = temp_aes;

        /* Third row (row 2) shifted left by two */
        temp_aes = block[2];
        block[2] = block[10];
        block[10] = temp_aes;
        temp_aes = block[14];
        block[14] = block[6];
        block[6] = temp_aes;

        /* Fourth row (row 3) shifted left by three (or right by one) */
        temp_aes = block[3];
        block[3] = block[15];
        block[15] = block[11];
        block[11] = block[7];
        block[7] = temp_aes;

		if (_roundCounter != 1) {
            unsigned char aux, aux1, aux2, aux3;
			/* Mix column */
			for (i_aes=0 ; i_aes<16 ; i_aes+=4) {
				aux1= block[i_aes+0] ^ block[i_aes+1];
				aux3= block[i_aes+2] ^ block[i_aes+3];
				aux = aux1 ^ aux3;
				aux2= block[i_aes+2] ^ block[i_aes+1];

				aux1 = xtime(aux1);
				aux2 = xtime(aux2);
				aux3 = xtime(aux3);

				block[i_aes+0] = aux ^ aux1 ^ block[i_aes+0];
				block[i_aes+1] = aux ^ aux2 ^ block[i_aes+1];
				block[i_aes+2] = aux ^ aux3 ^ block[i_aes+2];
				block[i_aes+3] = block[i_aes+0] ^ block[i_aes+1] ^ block[i_aes+2] ^ aux;
			}
		}
		/* Encode key schedule */
        key[0] ^= STable[key[13]];
        key[1] ^= STable[key[14]];
        key[2] ^= STable[key[15]];
        key[3] ^= STable[key[12]];

        key[0] ^= _rcon;
        _rcon = xtime(_rcon);

        /* Column 2 */
        key[4] ^= key[0];
        key[5] ^= key[1];
        key[6] ^= key[2];
        key[7] ^= key[3];

        /* Column 3 */
        key[8]^=key[4];
        key[9]^=key[5];
        key[10]^=key[6];
        key[11]^=key[7];

        /* Column 4 */
        key[12] ^= key[8];
        key[13] ^= key[9];
        key[14] ^= key[10];
        key[15] ^= key[11];

		/*   key addition */
		for (i_aes=0 ; i_aes<AES_BLOCK_SIZE ; i_aes++) {
			block[i_aes] ^= key[i_aes];
		}

		_roundCounter--;
	}
	while (_roundCounter != 0);
}
