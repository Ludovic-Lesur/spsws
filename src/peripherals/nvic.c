 ...
 * @param irq_idx: 	Interrupt number (use enum defined in 'nvic.h').
 * @return: 		None.
 */
void NVIC_enable_interrupt(NVIC_interrupt_t irq_idx) {
	NVIC -> ISER = (0b1 << (irq_idx & 0x1F));
}
...
void NVIC_set_priority(NVIC_interrupt_t irq_idx, uint8_t priority) {
	...
	// Reset bits.
	NVIC -> IPR[(irq_idx >> 2)] &= ~(0xFF << (8 * (irq_idx % 4)));
	// Set priority.
	NVIC -> IPR[(irq_idx >> 2)] |= ((priority << 6) << (8 * (irq_idx % 4)));
}
